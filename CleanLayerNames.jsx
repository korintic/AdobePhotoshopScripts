...
var ignoreGrps = true;
var ignoreCustomLayerNames = false; //Ignore layer names not starting with the photoshop defaults for that layer type
...
var adjustmentLayerDefaultNames = ["Brightness/Contrast", "Levels", "Curves", "Exposure", "Vibrance", "Hue/Saturation", "Invert", "Posterize", "Threshold"];
var adjustmentLayerLongDefaulNames = ["Color Balance", "Photo Filter", "Channel Mixer", "Color Lookup", "Gradient Map", "Selective Color"];

var shapeLayerDefaultNames = ["Rectangle", "Ellipse","Polygon", "Shape"];
var shapeLayerLongDefaultNames = ["Color Fill","Gradient Fill","Pattern Fill", "Rectangle", "Rounded Rectangle", "Ellipse","Polygon", "Shape"];
...
function CleanLayerNames() {
    ...
    for (var i = 0; i < selectedLayersIndices.length; i++) {
        ...
        if(!doc.activeLayer.isBackgroundLayer) {
            ...
            var isCustomLayerName = true;
            if(ignoreCustomLayerNames) {
                var nameStart = (doc.activeLayer.name.split(" ")[0]);
                var longNameStart = (doc.activeLayer.name.split(" ")[0]+" "+doc.activeLayer.name.split(" ")[1]);
                if((doc.activeLayer.name.startsWith("Layer") && doc.activeLayer.kind === LayerKind.NORMAL)) {
                    isCustomLayerName = false;
                }
                if((new RegExp(shapeLayerLongDefaultNames.join("|")).test(longNameStart)) && shapeFillAndPatternLayer[doc.activeLayer.kind.toString().split(".")[1]] !== undefined) {
                    isCustomLayerName = false;
                }
                else if((new RegExp(shapeLayerDefaultNames.join("|")).test(nameStart)) && shapeFillAndPatternLayer[doc.activeLayer.kind.toString().split(".")[1]] !== undefined) {
                    isCustomLayerName = false;
                }
                if((new RegExp(adjustmentLayerLongDefaulNames.join("|")).test(longNameStart)) && adjustmentLayer[doc.activeLayer.kind.toString().split(".")[1]] !== undefined) {
                    isCustomLayerName = false;
                }
                else if((new RegExp(adjustmentLayerDefaultNames.join("|")).test(nameStart)) && adjustmentLayer[doc.activeLayer.kind.toString().split(".")[1]] !== undefined) {
                    isCustomLayerName = false;
                }
                else if((doc.activeLayer.name.startsWith("Black & White") && adjustmentLayer[doc.activeLayer.kind.toString().split(".")[1]] !== undefined)) {
                    isCustomLayerName = false;
                }
                if(isCustomLayerName) {
                    continue;
                }
            }
